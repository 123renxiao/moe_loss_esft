...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        # [新增] 1. 提取 _tag，防止后续处理报错或丢失
        tags = [f.pop("_tag", None) for f in features]
        ...
        # [新增] 2. 将 _tag 放回 features，转为 Tensor
        if any(t is not None for t in tags):
            valid_tags = [t if t is not None else -1 for t in tags]
            features["_tag"] = torch.tensor(valid_tags, dtype=torch.long)
